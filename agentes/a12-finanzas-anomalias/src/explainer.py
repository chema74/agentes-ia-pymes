...
class FraudExplainer:
    ...
    def generate_report(self, transactions_flagged: pd.DataFrame) -> str:
        ...
        report.append(
            f"**Tasa de alerta:** {transactions_flagged['prediction'].mean() * 100:.2f}%\n\n"
        )

        top_suspicious = transactions_flagged[transactions_flagged["prediction"] == 1].nlargest(
            5, "confidence"
        )

        report.append("## 🔝 Top 5 Transacciones Sospechosas\n")
        for _, txn in top_suspicious.iterrows():
