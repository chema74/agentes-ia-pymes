...
def main():
    ...
    parser.add_argument(
        "--model", type=str, default="models/fraud_detector.pkl", help="Ruta del modelo"
    )
    ...
    if len(alerts) > 0:
        print("\n🚨 Top 3 alertas:")
        for _, alert in alerts.nlargest(3, "confidence").iterrows():
            explanation = explainer.explain_transaction(alert)
            print(
                f"   • {alert['transaction_id']}: €{alert['amount']:.2f} - {explanation['explanation']}"
            )
